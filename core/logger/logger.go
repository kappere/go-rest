...
func (l *LogFile) formatHeader(buf *[]byte, prefix string, t time.Time, file string, goroutine string) {
	*buf = append(*buf, prefix...)
	...
	// itoa(buf, day, 2)
	// *buf = append(*buf, ' ')
	*buf = append(*buf, FormatDateTime(t)...)
	*buf = append(*buf, ' ')
	if file != "" {
...
func Debug(format string, v ...interface{}) {
	if logFile.level >= DebugLevel {
		header := logFile.formatMsgHeader(2, plain(DEBUG_PREFIX), "")
...
func Info(format string, v ...interface{}) {
	if logFile.level >= InfoLevel {
		header := logFile.formatMsgHeader(2, plain(INFO_PREFIX), "")
		_ = logger.Output(2, NewMessageString(header+fmt.Sprintf(format, v...)))
	}
}

func Log(prefix string, callDepth int, format string, v ...interface{}) {
	if logFile.level >= InfoLevel {
		header := logFile.formatMsgHeader(callDepth, plain(prefix), "")
		_ = logger.Output(callDepth, NewMessageString(header+fmt.Sprintf(format, v...)))
	}
}
...
func logSql(format string, v ...interface{}) {
	if logFile.level >= InfoLevel {
		...
		header := logFile.formatMsgHeader(2, plain(INFO_PREFIX), codeLine)
...
func Warn(format string, v ...interface{}) {
	if logFile.level >= WarnLevel {
		header := logFile.formatMsgHeader(2, plain(WARN_PREFIX), "")
		_ = logger.Output(2, NewMessageString(header+fmt.Sprintf(format, v...)))
	}
}
...
func Error(format string, v ...interface{}) {
	if logFile.level >= ErrorLevel {
		header := logFile.formatMsgHeader(2, plain(ERROR_PREFIX), "")
		_ = logger.Output(2, NewMessageString(header+fmt.Sprintf(format, v...)))
	}
}
...
func Fatal(format string, v ...interface{}) {
	if logFile.level >= FatalLevel {
		header := logFile.formatMsgHeader(2, plain(FATAL_PREFIX), "")
		_ = logger.Output(2, NewMessageString(header+fmt.Sprintf(format, v...)))
	}
}
...
func (me *LogFile) removeExpireFile() {
	// TODO
}
...
func (me LogFile) Write(buf []byte) (n int, err error) {
	...
	default: // 默认按天  ByDay
		if logFile.logTime+86400 < now.Unix() || logFile.logTime > now.Unix() {
			...
			if logFile.saveDays >= 0 {
				logFile.removeExpireFile()
			}
		}
	}
